 ...
 *  A class providing actual implementation of Crypto interface in split-phase manner.
 *  @version   1.0
 * 	@date      2012-2014
 */
...
components:
    ICryptoRaw  *c_cryptoraw;
    IKeyDistrib *c_keydistrib;
    IAES        *c_aes;
    ISharedData *c_shareddata;
...
public:
    ...
    CryptoP(ICryptoRaw cryptoraw, IKeyDistrib keydistrib, IAES aes, ISharedData shaeddata)
    {
        i_crypto = new Crypto;

        this->c_cryptoraw   = cryptoraw;
        this->c_keydistrib  = keydistrib
        this->c_aes         = aes;
        this->c_shareddata  = shaeddata;
    }
...
class Crypto: public ICrypto{
    error_t macBufferForNodeB( node_id_t nodeID, uint8_t* buffer, uint8_t offset, uint8_t* pLen){        
        error_t status = SUCCESS;
        
        pl_log_i( TAG,"  macBufferForNodeB called.\n");
        //return status;
        if((status = call c_keydistrib.getKeyToNodeB( nodeID, &m_key1)) == SUCCESS){
	    //return status;
            status = call c_cryptoraw.macBuffer(m_key1, buffer, offset, pLen, buffer + offset + *pLen);
    ...
    error_t macBufferForBSB( uint8_t* buffer, uint8_t offset, uint8_t* pLen){		
        error_t status = SUCCESS;
        
        pl_log_i( TAG,"  macBufferForBSB called.\n"); 
        
        if((status = call c_keydistrib.getKeyToBSB(&m_key1)) == SUCCESS){	
            status = call c_cryptoraw.macBuffer(m_key1, buffer, offset, pLen, buffer + offset + *pLen);
            *pLen = *pLen + MAC_LENGTH;
        } else {
    ...
    error_t verifyMacFromNodeB( node_id_t nodeID, uint8_t* buffer, uint8_t offset, uint8_t* pLen){
        error_t status = SUCCESS;        
	   ...
	   pl_log_e( TAG,"  macBufferForNodeB failed, key to node not found.\n"); 
        ...
        status = call c_cryptoraw.verifyMac(m_key1, buffer,  offset, pLen);
        return status;
    }	
    
    error_t verifyMacFromBSB( uint8_t* buffer, uint8_t offset, uint8_t* pLen){
       error_t status = SUCCESS;        
    ...
    error_t protectBufferForNodeB( node_id_t nodeID, uint8_t* buffer, uint8_t offset, uint8_t* pLen){
        error_t status = SUCCESS;	
        ...
        status = call c_cryptoraw.protectBufferB( m_key1, buffer, offset, pLen);
        
        return status;
    }	
    
    error_t unprotectBufferFromNodeB( node_id_t nodeID, uint8_t* buffer, uint8_t offset, uint8_t* pLen){		
        error_t status = SUCCESS;		
        ...
        status = call c_cryptoraw.unprotectBufferB( m_key1, buffer, offset, pLen);
        return status;
    }		
    
    error_t protectBufferForBSB( uint8_t* buffer, uint8_t offset, uint8_t* pLen){
        error_t status = SUCCESS;	
    ...
    error_t unprotectBufferFromBSB( uint8_t* buffer, uint8_t offset, uint8_t* pLen){
        error_t status = SUCCESS;		
...
#define min( a, b ) ( ((a) < (b)) ? (a) : (b) )
		...
		status = call c_cryptoraw.deriveKeyB(m_key1, m_buffer, 0, BLOCK_SIZE, m_key2);
		if(status != SUCCESS){                
    ...
    error_t hashDataB(uint8_t* buffer, uint8_t offset, uint8_t len, uint8_t* hash){
        error_t status = SUCCESS;
        uint8_t i;
        uint8_t j;
        uint8_t numBlocks;
    ...
    error_t hashDataShortB( uint8_t* buffer, uint8_t offset, uint8_t len, uint32_t* hash){
        uint8_t tempHash[HASH_LENGTH];
        uint8_t status;
        //uint8_t i;
        ...
        if((status = call hashDataB(buffer, offset, len, tempHash)) != SUCCESS){
    ...
    error_t verifyHashDataB( uint8_t* buffer, uint8_t offset, uint8_t len, uint8_t* hash){
        error_t status = SUCCESS;
    ...
    error_t verifyHashDataShortB( uint8_t* buffer, uint8_t offset, uint8_t len, uint32_t hash){
        error_t status = SUCCESS;
        uint32_t tempHash;

        pl_log_i( TAG," verifyHashDataB called.\n");
        if((status = call hashDataShortB(buffer, offset, len, &tempHash)) != SUCCESS){
            pl_log_e( TAG," verifyHashDataB failed to calculate hash.\n"); 
        }
        if(tempHash != hash){
            pl_log_e( TAG," verifyHashDataB hash not verified.\n"); 
    ...
    error_t computeSignature( PRIVACY_LEVEL privacyLevel, uint16_t lenFromRoot, Signature_t* signature){
	...
	PPCPrivData_t* ppcPrivData = NULL;
        ...
        ppcPrivData = call c_shareddata.getPPCPrivData();
        if(ppcPrivData == NULL){
	 ...
	 if(root == NULL){
	    ...
	    status = call hashDataB( tmpSignature, 0, HASH_LENGTH, tmpSignature);
	    if (status != SUCCESS){
    ...
    error_t verifySignature( uint8_t* buffer, uint8_t offset, PRIVACY_LEVEL level, uint16_t counter, Signature_t* signature){
        uint8_t i;
        uint8_t status = SUCCESS;
	...
	if(buffer == NULL){
	    ...
	    status = call hashDataB( tmpSignature, 0, SIGNATURE_LENGTH, tmpSignature);
	    if (status != SUCCESS){
