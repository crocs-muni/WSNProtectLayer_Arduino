 ...
 *  A class providing actual implementation of CryptoRaw interafce in split-phase manner.
 *  @version   1.0
 * 	@date      2012-2014
 */
...
//  CryptoRaw interface
//
class CryptoRaw: public ICryptoRaw {
    error_t selfTest(){
...
*/
    ...
    error_t unprotectBufferB( PL_key_t* key, uint8_t* buffer, uint8_t offset, uint8_t* pLen){
        ...
        if((status = call decryptBufferB( key, buffer, offset, *pLen)) != SUCCESS){
            pl_log_e( TAG, "  unprotectBufferB encrypt failed.\n");
            return status;      
        }
        if((status = call verifyMac( key, buffer, 0, pLen)) != SUCCESS){            
        ...
        call decryptBufferB( key, buffer, offset, *pLen);
        if((status = call verifyMac( key, buffer, 0, pLen)) == SUCCESS){
    ...
    error_t protectBufferB( PL_key_t* key, uint8_t* buffer, uint8_t offset, uint8_t* pLen){
        error_t status = SUCCESS;
...
*/        
        ...
        if((status = call macBuffer(key, buffer, 0, pLen, buffer + *pLen)) != SUCCESS){
            pl_printf("CryptoP:  protectBufferForBSB mac failed.\n");
            return status;
        }
        if((status = call encryptBufferB( key, buffer, offset, *pLen)) != SUCCESS){
    ...
    error_t hashDataBlockB( uint8_t* buffer, uint8_t offset, PL_key_t* key, uint8_t* hash){
        error_t status = SUCCESS;       
        uint8_t i;
...
*/
        ...
        call m_aes->keyExpansion( m_exp, (uint8_t*) key->keyValue);        
        call m_aes->encrypt(buffer + offset, m_exp, hash);
    ...
    error_t deriveKeyB(PL_key_t* masterKey, uint8_t* derivationData, uint8_t offset, uint8_t len, PL_key_t* derivedKey) {
        ...
        call m_aes->keyExpansion( m_exp, (uint8_t*)(masterKey->keyValue));
        call m_aes->encrypt( derivationData + offset, m_exp, (uint8_t*)(derivedKey->keyValue));
...
*/
        ...
        status = call macBuffer(key, buffer, offset, &macLength, mac); //calculate new mac    
    ...
    error_t macBuffer(PL_key_t* key, uint8_t* buffer, uint8_t offset, uint8_t* pLen, uint8_t* mac){
        uint8_t i;
        uint8_t j;
...
*/        
        call m_aes->keyExpansion( m_exp, key->keyValue);
    ...
    error_t encryptBufferB(PL_key_t* key, uint8_t* buffer, uint8_t offset, uint8_t len) {
        uint8_t i;
        uint8_t j;
...
*/        
        ...
        for(i = 0; i < (len / BLOCK_SIZE) + 1; i++){
            ...
            call m_aes->encrypt( plainCounter, m_exp, encCounter);
    ...
    error_t decryptBufferB(PL_key_t* key, uint8_t* buffer, uint8_t offset, uint8_t len) {
        //for counter mode encrypt is same es decrypt
        return call encryptBufferB(key, buffer, offset, len);
    }
}
